 ...
 */
public class JiPrologBenchmarkRunner extends AbstractBenchmarkRunner implements BenchmarkRunner {
	...
	public static class ExecutionPlan {
		...
		public void setUp() {
			...
			PrologProvider provider = Prolog.getProvider(JiProlog.class);
...
//							provider.newStructure(provider.newStructure(p, "*", provider.newInteger(100)), "/", a)));
	...
	public static void main(String[] args) throws RunnerException {
		...
		builder.include(JiPrologBenchmarkRunner.class.getSimpleName());
		Options opt = builder.build();
		Collection<RunResult> run = new Runner(opt).run();
		new JiPrologBenchmarkRunner().output(run);
